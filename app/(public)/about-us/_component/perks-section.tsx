...
export const PerksSection = () => {
  return (
    <section className="py-20 md:py-24 lg:py-48 px-2 md:px-8 lg:px-24 mx-auto relative z-10">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-3 justify-between lg:mt-40 md:mt-20 mt-16 gap-6 md:gap-2 lg:gap-16 max-w-xs md:max-w-fit mx-auto">
        {PERKS.map((item: string, index: number) => (<GoalCard index={index} text={item} key={item}/>))}
      </div>

      <div className="pt-16 lg:pt-24 gap-12 grid grid-cols-2 md:grid-cols-4">
        {RESULT_PROGRESS.map(({ progress, text }) =>
          (
            <div key={text} className="w-32 lg:w-60 flex flex-col lg:gap-10 items-center mx-auto md:w-full">
              ...
              <div className="font-unbounded text-2xl lg:text-4xl mt-6 lg:mt-10">
                {text}
              </div>
            </div>
          ))}
